...
export async function loadSprites(): Promise<SpriteManager> {
  ...
  const [
    ...
    loadImageOptional('/assets/furniture/COFFEE_TABLE/COFFEE_TABLE_FRONT.png'),
    ...charPromises,
  ]);
  ...
  return {
    characters: sheets,
    furniture: {
      desk: { front: deskFront },
      pc: { front: pcFront, back: pcBack },
      chair: { front: chairFront, back: chairBack },
