...
from routers.workers import get_worker, list_workers  # noqa: E402
...
class FakeTmuxBackend:
    ...
    def capture(self, session, lines=200):
        return f"{self.capture_text}:{session}:{lines}"
...
class DashboardServerTests(unittest.TestCase):
    ...
    def test_workers_list_and_detail_match_frontend_contract(self):
        ...
        detail = get_worker(request, "awg-worker", lines=40)
        self.assertEqual(detail["recentOutput"], "terminal output:awg-worker:40")
        self.assertEqual(monitor._last_capture["awg-worker"], detail["recentOutput"])
