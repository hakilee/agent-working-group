...
Read-only worker state report for one AWG role queue.

The helper observes queue state with non-consuming AWG CLI commands and prints
an advisory recommendation category. It never claims work and never changes
queue files.
...
category=$(printf '%s' "$status_json" | python3 -c '
import json
import sys
...
if dead:
    ...
    print("idle")
')
reason=$(printf '%s' "$status_json" | python3 -c '
import json
import sys
...
processing = int(status.get("processing", 0))
dead = int(status.get("dead", 0))
print(f"pending={pending} processing={processing} dead={dead}")
')

printf 'Worker state read-only report\n'
printf 'Role: %s\n' "$ROLE"
printf 'Recommendation: advisory only; operator decides next action\n\n'
