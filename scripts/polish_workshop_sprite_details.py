...
def save_grass_variant(index: int, base: Color, accent: Color, blade: Color) -> None:
    img = Image.new("RGBA", (16, 16), base)
    pix = img.load()
    for y in range(16):
        for x in range(16):
            pix[x, y] = texture_noise_color(base, x + index * 5, y + index * 3, 8)
    # Cross-hatched blade pattern keeps the tile tileable while reading as grass.
    for x, y in [(2, 3), (5, 1), (8, 4), (11, 2), (14, 5), (3, 8), (7, 7), (10, 9), (13, 11), (1, 11), (4, 13), (9, 12), (12, 14), (6, 14)]:
        pix[x, y] = blade
        if y + 1 < 16:
            pix[x, y + 1] = texture_noise_color(accent, x, y, 6)
    for x, y in [(3, 5), (9, 3), (12, 7), (5, 10), (8, 13)]:
        pix[x, y] = accent
    img.save(ASSETS / "floors" / f"floor_{index}.png")


def save_deck_variant(index: int, base: Color, plank: Color, seam: Color) -> None:
    img = Image.new("RGBA", (16, 16), base)
    pix = img.load()
    for y in range(16):
        for x in range(16):
            pix[x, y] = texture_noise_color(plank, x + index * 11, y + index * 7, 3)
    # Three horizontal planks separated by darker seams.
    for y in (0, 5, 10, 15):
        for x in range(16):
            pix[x, y] = seam
    # Plank end joints, offset so the pattern doesn't read as a single panel.
    for x in (4, 12):
        for y in range(1, 5):
            pix[x, y] = seam
    for x in (8,):
    ...
    for x in (3, 13):
        for y in range(11, 15):
            pix[x, y] = seam
    # A few nail/dot accents.
    for x, y in [(2, 1), (9, 6), (14, 11), (5, 11)]:
        pix[x, y] = base
    img.save(ASSETS / "floors" / f"floor_{index}.png")
